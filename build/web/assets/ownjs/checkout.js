...
async function loadCheckoutData() {
    ...
    if (response.ok) {
        ...
        if (json.status) {
            ...
            // cart-details
            let st_tbody = document.getElementById("checkout-body");
            let st_item_tr = document.getElementById("checkout-item-r");
            let st_subtotal_tr = document.getElementById("checkout-subtotal-r");
            let st_order_shipping_tr = document.getElementById("checkout-order-shipping-r");
            let st_order_total_tr = document.getElementById("checkout-order-total-r");
            ...
            cartItems.forEach(cart => {
                ...
                st_item_tr_clone.querySelector("#product-title")
                        .innerHTML = cart.product.title;
                st_item_tr_clone.querySelector("#product-qty")
                        .innerHTML = cart.qty;
                item_count += cart.qty;
                let item_sub_total = Number(cart.qty) * Number(cart.product.price);

                st_item_tr_clone.querySelector("#product-price")
            ...
            st_subtotal_tr.querySelector("#product-subtotal")
                    .innerHTML = "$" + new Intl.NumberFormat(
                            "en-US",
                            {minimumFractionDigits: 2})
                    .format(total);
            ...
            city_select.addEventListener("change", (e) => {
                ...
                if (cityName === "Colombo") {
                    ...
                    // out of colombo
                    shipping_charges = item_count * deliveryTypes[2].price;
                }

                st_order_shipping_tr.querySelector("#product-shipping-cost")
                        .innerHTML = new Intl.NumberFormat(
                                "en-US",
                                {minimumFractionDigits: 2})
                ...
                st_order_total_tr.querySelector("#order-total-amount")
                        .innerHTML = "$" + new Intl.NumberFormat(
                                "en-US",
                                {minimumFractionDigits: 2})
